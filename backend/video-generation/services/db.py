...
def create_job(job_id, image_url, prompt, params, created_at):
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO video_jobs (id, status, image_url, prompt, params, created_at)
           VALUES (%s, 'pending', %s, %s, %s, %s)""",
        (job_id, image_url, prompt, json.dumps(params), created_at),
    )
    conn.commit()
    cur.close()
    conn.close()
