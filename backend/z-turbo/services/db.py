...
def init_db():
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS zturbo_generation_jobs (
            id TEXT PRIMARY KEY,
...
def create_job(job_id, prompt, params, created_at):
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO zturbo_generation_jobs (id, status, prompt, params, created_at)
           VALUES (%s, 'pending', %s, %s, %s)""",
        (job_id, prompt, json.dumps(params), created_at),
    )
    conn.commit()
    cur.close()
    conn.close()
...
def get_job(job_id):
    ...
    cur.execute("SELECT * FROM zturbo_generation_jobs WHERE id = %s", (job_id,))
    row = cur.fetchone()
    cur.close()
    conn.close()
    if not row:
