...
def review(image_bytes: bytes, subject: str) -> dict:
    prompt = (
        ...
        f"2. CORRECT USAGE & PHYSICAL PLAUSIBILITY — Is the '{subject}' being worn, held, or "
        f"used in a physically correct and natural way? Ask yourself: does this make real-world "
        f"sense? For example — if the subject is a bag, is it being carried correctly (not clipped "
        f"to thin air, not inside-out, strap visible and natural)? If it's a shoe, is it worn on a "
        f"foot in a believable pose? If it's an action (tying a lace, zipping a jacket), does the "
        f"depicted motion look anatomically and physically plausible? Wrong orientation, impossible "
        f"usage, the subject floating detached from the character, or implausible interactions are "
        f"all defects. Be specific in the reason about what is physically wrong.\n\n"
        ...
        f"If the score is below 7.0, return `suggested_prompt_adjustments` — write this as direct "
        f"prompt instructions the writer should use verbatim or closely follow. Be very specific:\n"
        f"- If subject is too small: e.g. 'extreme close-up shot, {subject} filling 60% of the frame, "
        f"shot with 85mm lens at f/1.8'\n"
        f"- If subject is obscured: e.g. 'character holds {subject} extended forward toward camera, "
        f"both hands visible gripping it, nothing overlapping it'\n"
        f"- If physical interaction is wrong (floating, wrong grip, impossible pose): describe the "
        f"correct physical interaction in anatomical detail — e.g. exact hand position, which fingers "
        f"contact the object, how weight is distributed, what body part the item rests on. "
        f"Do not just say 'hold it correctly' — write what the prompt should actually say.\n"
        f"- If AI artefacts: e.g. 'minimal background, plain studio wall, single key light, "
        f"no reflections, no complex patterns'\n"
        f"- If body deformity: e.g. 'simple standing pose, both hands at sides, avoid gestures, "
        f"avoid motion blur'\n\n"
