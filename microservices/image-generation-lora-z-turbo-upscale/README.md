...
scp your_lora.safetensors root@<pod-ip>:/workspace/models/loras/

# Or from within the pod using wget/huggingface-cli etc.
```
...
> On a Serverless worker, the volume mounts at `/runpod-volume`. The `extra_model_paths.yaml` maps all model directories (including `models/loras/` and `models/SEEDVR2/`) automatically.
...
| `denoise` | No | `1.0` | Denoise strength for the LoRA generation pass |
| `upscale_denoise` | No | `0.8` | Denoise strength for the latent upscale refine pass |
| `scale_by` | No | `1.25` | Latent upscale factor before SeedVR2 |
| `upscale_resolution` | No | `2560` | Target resolution for SeedVR2 upscale |
| `seed` | No | random | Random seed (shared across all sampling stages) |
